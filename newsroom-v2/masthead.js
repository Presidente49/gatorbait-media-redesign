...
function render(posts){
 ...
 var franz=pickWriter(posts,'franz');
 var buddy=pickWriter(posts,'buddy');
 ...
 function section(name,arr){if(!arr||!arr.length)return'';var use=arr.slice(0,3);return'<section class="gbm-v2-section"><div class="gbm-v2-section-head"><h2>'+name+'</h2><a href="/gatorbait-media-blogs">More '+name+'</a></div><div class="gbm-v2-section-grid">'+use.map(sectionCard).join('')+'</div></section>';}
 function writerCard(p,label,heading,cta){if(!p)return'';return'<article>'+storyLink(p,'')+'<span class="gbm-v2-kicker">'+esc(label)+'</span><h2>'+esc(heading)+'</h2><h3>'+esc(p.title)+'</h3><p>'+excerpt(p,125)+'</p><span class="gbm-v2-read">'+esc(cta)+' →</span></a></article>';}
 var sportsNav=[['Football',football],['Recruiting',recruiting],['Basketball',basketball],['Baseball',baseball]].filter(function(pair){return pair[1].length>0;}).map(function(pair){return'<a href="#'+pair[0].toLowerCase()+'">'+pair[0]+'</a>';}).join('');
 ...
 '<header class="gbm-v2-header"><div class="gbm-v2-nameplate"><a href="/" class="gbm-v2-brand"><img src="'+LOGO+'" alt="GatorBait Media"></a><div class="gbm-v2-edition"><strong>Florida Gators Newsroom</strong><span>Independent coverage since 1979</span></div><div class="gbm-v2-actions"><a href="/pricing-plans">Join</a><a href="'+STORE+'">Store</a></div></div><nav class="gbm-v2-nav" aria-label="Sections">'+sportsNav+'<a href="/the-buddy-martin-show">GatorBait TV</a><a href="/magazine">Magazine</a></nav></header>'+
 ...
 '<section class="gbm-v2-voices">'+writerCard(franz,'Franz Beard',franz&&/thoughts of the day/i.test(franz.title)?'Thoughts of the Day':'Latest Column','Read Franz')+writerCard(buddy,'Buddy Martin','From the Founder','Read Buddy')+'<aside class="gbm-v2-most"><div class="gbm-v2-block-title"><h2>More Coverage</h2><span>Recent</span></div>'+posts.slice(3,7).map(function(p,i){return'<a href="'+esc(p.link)+'"><b>'+(i+1)+'</b><span>'+esc(p.title)+'</span></a>';}).join('')+'</aside></section>'+
...
function start(){if(!window.GBMV2Data){fail(new Error('V2 data adapter unavailable'));return;}window.GBMV2Data.load().then(render).catch(fail);}
