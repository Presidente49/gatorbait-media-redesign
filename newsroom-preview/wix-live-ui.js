...
function accountControls(){
 var old=document.querySelector('.gbm-account-tools');if(old)old.remove();
 if(home()){var root=document.getElementById('gbm-live');if(root){var tools=document.createElement('div');tools.className='gbm-account-tools';tools.innerHTML='<button type="button" data-gbm-account>Manage membership / cancel</button><a href="'+mail('Unsubscribe me from GatorBait Weekly','Please unsubscribe this email address from GatorBait marketing/newsletter emails.\n\nEmail to remove:')+'">Newsletter unsubscribe</a>';var f=root.querySelector('footer');if(f)root.insertBefore(tools,f);}}
 else if(post()&&!document.getElementById('gbm-account-fab')){var b=document.createElement('button');b.id='gbm-account-fab';b.type='button';b.textContent='Account & preferences';document.body.appendChild(b);}
}
