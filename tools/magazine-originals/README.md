...
Canonical source inventory on main: tools/magazine-originals
Original validation branch retained as evidence: tools/magazine-originals-20260924
...
## Main-branch operating note — September 24, 2026

This source inventory is mirrored onto current `main` so Claude, Codex, ChatGPT and other bounded workers discover the same original-tool set. The historical native-validation receipts remain evidence from the isolated runner. Source presence on `main` does **not** authorize executing security-hold dependencies on a credential-bearing host. Colorlib is also pinned at `vendor/colorlib-email-templates` because it is the production newsletter design library; the duplicate source pin here preserves the complete original intake exactly.
