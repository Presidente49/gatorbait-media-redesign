(function(){
 ...
 function sync(){
  ...
  if(p==='/contact'){var submit=document.querySelector('#comp-lii7mqe42 button[type=submit]');if(submit){submit.setAttribute('aria-label','Send message');var label=submit.querySelector('span');if(label&&label.textContent.trim()==='>')label.textContent='Send message';}}
 ...
 function schedule(){[0,200,800,1800].forEach(function(ms){setTimeout(sync,ms);});}
