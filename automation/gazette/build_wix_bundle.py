#!/usr/bin/env python3
"""Package the original compiled Gazette into a scoped Wix presentation bundle.
No new theme recreation, iframe, head metadata, Google code, or business integration.
"""
...
# Rewrite selectors, not the surrounding Wix document. Keep font faces and tokens.
def selectors(tokens):
    ...
    for selector in groups:
        ...
        if re.match(r'^(?:html|body|:root|:host)(?=[\s.\[:#>+~]|$)', selector):
            selector = re.sub(r'^(?:html|body|:root|:host)', SCOPE, selector, count=1)
...
def scoped_css(text):
    ...
    for rule in tinycss2.parse_stylesheet(text, skip_comments=True, skip_whitespace=True):
        if rule.type == 'qualified-rule':
            result.append(selectors(rule.prelude) + '{' + tinycss2.serialize(rule.content) + '}')
        elif rule.type == 'at-rule':
            prelude = tinycss2.serialize(rule.prelude)
            if rule.lower_at_keyword == 'layer':
                prelude = re.sub(r'\b(properties|theme|base|utilities)\b', r'gbgz-\1', prelude)
            if rule.content is None:
                result.append('@' + rule.at_keyword + prelude + ';')
            elif rule.lower_at_keyword in ('media', 'supports', 'layer', 'container'):
                result.append('@' + rule.at_keyword + prelude + '{' + scoped_css(tinycss2.serialize(rule.content)) + '}')
            else:
                result.append('@' + rule.at_keyword + prelude + '{' + tinycss2.serialize(rule.content) + '}')
    return ''.join(result)

css = ''.join(scoped_css(s.text) for s in soup.head.select('style'))
for link in soup.head.select('link[rel=stylesheet]'):
    css += scoped_css((SOURCE / '_astro' / Path(link['href']).name).read_text())
...
# Existing unified Wix mobile shell remains the only mobile menu.
...
main = soup.select_one('main').extract()
main['id'] = 'gbgz-main'
...
nameplate.select_one('p span').string = 'Independent Florida Gators coverage'
...
contents.select_one('h2').clear()
contents.select_one('h2').string = 'Inside GatorBait'
rows = contents.select_one('ol')
row = rows.select_one('li').extract()
...
for comment in main.find_all(string=lambda text: text.__class__.__name__ == 'Comment'):
    comment.extract()
