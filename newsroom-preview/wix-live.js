...
function feed(){return fetch('/blog-feed.xml?gbm='+Date.now(),{credentials:'same-origin',cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('feed '+r.status);return r.text();}).then(function(xml){var doc=new DOMParser().parseFromString(xml,'application/xml');var items=Array.prototype.slice.call(doc.querySelectorAll('item'));return items.map(function(item){function t(tag){var n=item.querySelector(tag);return n?n.textContent:'';}var raw=t('pubDate'),d=raw?new Date(raw):new Date(0),link=local(t('link').trim()),pic=image(item)||IMAGE_FALLBACKS[slug(link)]||'';return{title:t('title').trim(),desc:t('description'),link:link,img:pic,who:creator(item),date:d};}).filter(function(p){return p.title&&/^\/post\//.test(p.link)&&!magazinePost(p.link);}).sort(function(a,b){return b.date.getTime()-a.date.getTime();}).slice(0,8);});}
...
'<section class="newsletter" aria-labelledby="gbm-newsletter"><div><span>THE EMAIL EDITION</span><h2 id="gbm-newsletter">GatorBait Weekly</h2><p>Request the newsletter by email. Your request is your consent to receive GatorBait emails, and every campaign includes an unsubscribe option.</p></div><a href="'+subscribe+'">Get GatorBait Weekly</a></section></main>'+
