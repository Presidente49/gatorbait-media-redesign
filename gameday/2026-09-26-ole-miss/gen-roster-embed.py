...
R = {
  ...
  'src': 'Official 2026 rosters from FloridaGators.com and OleMissSports.com, Sept. 26. Numbers can change on game day.',
...
function list(R,t){return'<ol>'+R.t[t].map(function(x){return'<li><b>'+e(x[0])+'</b><span>'+e(x[1])+'</span><i>'+e(x[2])+' · '+e(x[3])+'</i></li>';}).join('')+'</ol>';}
...
hit.innerHTML=[0,1].map(function(t){var m=R.t[t].filter(function(x){return x[0]===n;});return'<div class="'+(t?'om':'uf')+'"><b>'+e(R.names[t])+' · No. '+e(n)+'</b>'+(m.length?m.map(function(x){return e(x[1])+' · '+e(x[2])+' · '+e(x[3]);}).join('<br>'):'No player listed')+'</div>';}).join('');});
